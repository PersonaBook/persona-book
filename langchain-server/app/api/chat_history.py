...
async def get_all_chat_history(
    ...
    try:
        ...
        response_list = [
            ChatHistoryResponse.from_entity(history) for history in history_list
        ]

        return ChatHistoryListResponse(
            history=response_list, total_count=len(response_list)
        )
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"채팅 이력 조회 중 오류가 발생했습니다: {str(e)}"
        )
...
async def search_chat_history(
    ...
    try:
        ...
        return ChatHistorySearchResponse(
            history=response_list, total_count=len(response_list), query=query
        )
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"채팅 이력 검색 중 오류가 발생했습니다: {str(e)}"
        )
...
async def get_recent_chat_history(
    ...
    try:
        history_list = chat_history_repository.get_recent_chat_history(
            days=days, size=size
        )
        response_list = [
            ChatHistoryResponse.from_entity(history) for history in history_list
        ...
        raise HTTPException(
            status_code=500,
            detail=f"최근 채팅 이력 조회 중 오류가 발생했습니다: {str(e)}",
        )
...
async def get_chat_history_by_age_group(
    ...
    """연령대별 채팅 이력을 조회합니다."""
    try:
        history_list = chat_history_repository.get_chat_history_by_age_group(
            age_group, size=size
        )
        response_list = [
            ChatHistoryResponse.from_entity(history) for history in history_list
        ...
        raise HTTPException(
            status_code=500,
            detail=f"연령대별 채팅 이력 조회 중 오류가 발생했습니다: {str(e)}",
        )
...
async def delete_chat_history(history_id: str):
    ...
    try:
        ...
        if not success:
            raise HTTPException(
                status_code=404, detail="삭제할 채팅 이력을 찾을 수 없습니다"
            )

        return ChatHistoryDeleteResponse(
            message="채팅 이력이 성공적으로 삭제되었습니다", deleted_id=history_id
        )
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"채팅 이력 삭제 중 오류가 발생했습니다: {str(e)}"
        )
